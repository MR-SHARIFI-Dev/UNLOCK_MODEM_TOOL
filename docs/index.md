---
title: Universal Modem Unlocker
layout: default
---

<!-- Banner -->
<div align="center">
<img src="images/banner.png" alt="Universal Modem Unlocker Banner" width="100%"/>
</div>

<div align="center">
<img src="images/logo.png" width="140" alt="Logo"/>
<h1>🔓 Universal Modem Unlocker</h1>
<p><i>Professional LTE Modem Unlocking Tool</i></p>
</div>

---

## 🌟 Features / امکانات

### 🇬🇧 English
- 🔓 Unlock all major LTE modems (Huawei, Irancell, etc.)  
- ⚡ Portable & Fast EXE  
- 🛡 Digitally signed and secure  
- 🧩 Multi-brand support  
- 🖥 Windows 7 / 8 / 10 / 11 compatible  

### 🇮🇷 فارسی
- 🔓 آنلاک همه مودم‌های LTE (هواوی، ایرانسل و غیره)  
- ⚡ نسخه اجرایی پرتابل و سریع  
- 🛡 دارای امضای دیجیتال و ایمن  
- 🧩 پشتیبانی از برندهای مختلف  
- 🖥 سازگار با ویندوز 7 / 8 / 10 / 11  

---

## 📸 Screenshots / تصاویر

<div align="center">
<img src="images/screenshot1.png" width="300" style="margin:10px" alt="Screenshot 1"/>
<img src="images/screenshot2.png" width="300" style="margin:10px" alt="Screenshot 2"/>
</div>

---

## 📥 Download / دانلود
### 🇬🇧
Download the latest release from [GitHub Releases](https://github.com/YourUsername/YourRepo/releases).  
### 🇮🇷
برای دریافت نسخه جدید به بخش [Releases](https://github.com/YourUsername/YourRepo/releases) مراجعه کنید.

---

## 📚 Documentation / مستندات
- [Installation Guide / راهنمای نصب](installation.md)  
- [Compatibility / سازگاری دستگاه‌ها](compatibility.md)  
- [Security & Privacy / امنیت و حریم خصوصی](security.md)  
- [Statistics / آمار آنلاک](statistics.md)  

---

## 📊 Live Stats / آمار زنده

| Metric | Value |
|--------|-------|
| 🌍 Total Unlocks | 12,842 |
| ✅ Success Rate | 97.8% |
| 🖥 Active Countries | 18 |

---

## 💰 Support / حمایت مالی
- ☕ BuyMeACoffee: [Click Here](https://buymeacoffee.com/YourName)  
- ₿ Bitcoin: `YourBTCAddress`  
- 💎 USDT (TRC20): `YourWalletAddress`

---

## 🛡 Security / امنیت
- Digitally signed executable / دارای امضای دیجیتال  
- No personal data collected / بدون ذخیره اطلاعات شخصی  
- Virus-free & safe / بررسی شده و ایمن  

---

## 🔗 Connect / ارتباط
- Telegram: [@YourTelegram](https://t.me/YourTelegram)  
- YouTube: [YourChannel](https://youtube.com/YourChannel)  
- GitHub: [YourRepo](https://github.com/YourUsername/YourRepo)
